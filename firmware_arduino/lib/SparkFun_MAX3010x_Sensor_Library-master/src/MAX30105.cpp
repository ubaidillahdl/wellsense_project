...
static const uint8_t MAX30105_PARTICLECONFIG =
...
static const uint8_t MAX30105_PARTID =
...
float MAX30105::readTemperature() {
  ...
  uint8_t tempFrac = readRegister8(
      _i2caddr, MAX30105_DIETEMPFRAC); // Causes the clearing of the
...
uint8_t MAX30105::getRevisionID() { return revisionID; }
...
// Sample rate = 50
// Use the default setup if you are just getting started with the MAX30105
// sensor
void MAX30105::setup(byte powerLevel, byte sampleAverage, byte ledMode,
                     int sampleRate, int pulseWidth, int adcRange) {
  softReset(); // Reset all configuration, threshold, and data registers to POR
               // values
...
uint32_t MAX30105::getFIFORed(void) { return (sense.red[sense.tail]); }

// Report the next IR value in the FIFO
uint32_t MAX30105::getFIFOIR(void) { return (sense.IR[sense.tail]); }

// Report the next Green value in the FIFO
uint32_t MAX30105::getFIFOGreen(void) { return (sense.green[sense.tail]); }
...
uint16_t MAX30105::check(void) {
  ...
  if (readPointer != writePointer) {
    ...
    // than I2C_BUFFER_LENGTH I2C_BUFFER_LENGTH changes based on the platform. 64
    // bytes for SAMD21, 32 bytes for Uno. Wire.requestFrom() is limited to
